        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6 scrollbar-hide min-h-0">
            <AnimatePresence>
                {messages.map((msg, index) => (
                    ...
                    >
                        <div className={`p-2 rounded-full ${msg.role === 'user' ? 'bg-white/10 text-white' : 'bg-white/5 text-gray-300'}`}>
            ...
            {state === 'speaking' && (
                <motion.div
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    className="flex items-center gap-2 text-gray-400 text-xs font-mono ml-12"
                >
